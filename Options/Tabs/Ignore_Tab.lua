...
local function NotifyChanged()
  if _pendingOptionsRefresh then
    return
  end
  _pendingOptionsRefresh = true
  C_Timer.After(0.05, function()
    _pendingOptionsRefresh = false
    if O and _safeCall(O.OnOptionChanged) then
      return
    end
    if
      ns
      and (
        ...
        or _safeCall(ns.RefreshEverything)
        or _safeCall(ns.PushRender)
        or _safeCall(ns.RenderAll)
      )
    then
      return
    end
    if
      _G
      and (
        _safeCall(_G.ClickableRaidBuffs_Rebuild)
        or _safeCall(_G.ClickableRaidBuffs_ForceRefresh)
        or _safeCall(_G.ClickableRaidBuffs_PushRender)
      )
    then
      return
    end
...
local function NewCheckbox(parent)
  ...
  cb:SetScript("OnLeave", function(self)
    self:SetBackdropBorderColor(unpack(THEME.wellBR))
  end)
  return cb
end
...
local RANK_ATLAS = {
  ...
  [121] = "Professions-Icon-Quality-12-Tier1",
  [122] = "Professions-Icon-Quality-12-Tier2",
}
...
local function GetSpellIconSafe(spellID)
  ...
  if C_Spell and C_Spell.GetSpellInfo then
    ...
    if info and info.iconID then
      return info.iconID, false
    end
    if C_Spell.RequestLoadSpellData then
      C_Spell.RequestLoadSpellData(spellID)
      local info2 = C_Spell.GetSpellInfo(spellID)
      return (info2 and info2.iconID) or 134400, true
    end
  end
  return 134400, false
end
local function GetItemIconSafe(itemID)
  if C_Item and C_Item.GetItemIconByID then
    return C_Item.GetItemIconByID(itemID), false
  end
  if GetItemIcon then
    return GetItemIcon(itemID), false
  end
  return 134400, false
end
...
local function getCategorySource(cat)
  ...
  if type(d[cat]) == "table" then
    return d[cat]
  end
  local splitName = SPLIT_SOURCES[cat]
  if splitName and type(_G[splitName]) == "table" then
    return _G[splitName]
  end
  return nil
end
...
local function flattenSpells(src)
  ...
  for keyID, v in pairs(src) do
    if type(keyID) == "number" then
      ...
      local name
      if C_Spell and C_Spell.GetSpellInfo then
        name = (type(v) == "table" and (v.name or v.label))
          or (C_Spell.GetSpellInfo(spellID) or {}).name
          or tostring(spellID)
      else
      ...
      if not group then
        ...
        local found = false
        for i = 1, #group.ids do
          if group.ids[i] == keyID then
            found = true
            break
          end
        end
        if not found then
          group.ids[#group.ids + 1] = keyID
        end
      end
    end
  end
...
local function collectClassAbilitiesByClass()
  ...
  local function cweForClass(classKey)
    ...
    for _, it in ipairs(cweItems) do
      if map[it.spellID] then
        want[#want + 1] = it
      end
    end
    return want
  ...
  local function classIDFromKey(k)
    for id, key in pairs(CLASS_KEY_FOR_ID) do
      if key == k then
        return id
      end
    end
  ...
  if
    type(byClass) == "table"
    and type(root.RAID_BUFF_CLASS_ORDER) == "table"
    and type(root.RAID_BUFF_CLASS_LABELS) == "table"
  then
    for _, classKey in ipairs(root.RAID_BUFF_CLASS_ORDER) do
      local baseTbl = byClass[classKey] or {}
      local items, req = flattenSpells(baseTbl)
      requested = requested or req

      if classKey == "ROGUE" then
        items = mergeUnique(items, rogPoisItems)
      end
      if classKey == "SHAMAN" then
        items = mergeUnique(items, shamanShields())
        items = mergeUnique(items, cweForClass("SHAMAN"))
      end
      if classKey == "PALADIN" then
        items = mergeUnique(items, cweForClass("PALADIN"))
      end
      if classKey == "WARLOCK" then
        items[#items + 1] = makeHealthstoneEntry()
      end

      if #items > 0 then
        local cid = classIDFromKey(classKey) or 99
        tmp[#tmp + 1] =
          { cid = cid, block = { label = root.RAID_BUFF_CLASS_LABELS[classKey] or tostring(classKey), items = items } }
      end
    end
  elseif type(byClass) == "table" and next(byClass) then
    for classID = 1, 13 do
      ...
      if #items > 0 then
        tmp[#tmp + 1] =
          { cid = classID, block = { label = CLASS_NAMES[classID] or ("Class " .. tostring(classID)), items = items } }
      end
    end
  else
    local flat, reqAny = {}, false
    if type(root.ALL_RAID_BUFFS) == "table" then
      flat, reqAny = flattenSpells(root.ALL_RAID_BUFFS)
    end
    if #flat > 0 then
      outBlocks[#outBlocks + 1] = { label = "All Class Abilities", items = flat }
    end
    if reqAny and ns.Exclusions_RefreshNow then
      C_Timer.After(0.35, function()
        ns.Exclusions_RefreshNow(true)
      end)
    end
    return outBlocks
  end
  ...
  for i = 1, #tmp do
    outBlocks[#outBlocks + 1] = tmp[i].block
  end
  if requested and ns.Exclusions_RefreshNow then
    C_Timer.After(0.35, function()
      ns.Exclusions_RefreshNow(true)
    end)
  end
    ...
    if type(tbl) == "table" then
      for id, info in pairs(tbl) do
        if
          type(id) == "number"
          and type(info) == "table"
          and (info.type == "trinket" or info.topLbl == INVTYPE_TRINKET)
        then
...
O.RegisterSection(function(AddSection)
  AddSection("Ignore", function(content, Row)
    ...
    local bar = (ns.ScrollBar and ns.ScrollBar.Create)
        and ns.ScrollBar.Create(inner, { width = BAR_WIDTH, sliderWidth = BAR_WIDTH - 2, minThumbH = 24 })
      or nil
    if bar then
    ...
    local function ResetScrollToTop()
      ...
      if bar and bar.UpdateThumb then
    ...
    local function clearHeaders()
      ...
      for i = 1, #headerBtns do
        ...
        if pair then
          if pair.ignore then
            pair.ignore:Hide()
            pair.ignore:SetParent(nil)
          end
          if pair.enable then
            pair.enable:Hide()
            pair.enable:SetParent(nil)
          end
        end
      end
      if wipe then
    ...
    local function ApplyBulkExclusion(items, activeKeyForList, setOn)
      ...
      ApplyNow_NoReload()
      if ns.Exclusions_RefreshNow then
        ns.Exclusions_RefreshNow(true)
      end
      NotifyChanged()
    end
    ...
    local function makeLine(idx)
      ...
      line:SetScript("OnEnter", function(self)
        ...
        if self._tipSpell and ShowTipSpell(self._tipSpell) then
          return
        end
        ...
        if self._isItem then
          if self._id and self._id > 0 then
            GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
            GameTooltip:SetItemByID(self._id)
            GameTooltip:Show()
          end
        else
          local sid = self._spellID or self._id
          if sid and sid > 0 then
            if not ShowTipSpell(sid) and GameTooltip.SetSpellByID then
              GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
    ...
    local function configureLine(line, info, nameW, activeKey)
      ...
      if info.icon then
        line._icon:SetTexture(info.icon)
        line._icon:Show()
      else
        line._icon:Hide()
      end
      ...
      if info._comboHS then
        excluded = (exset[5512] or exset[224464]) and true or false
      end

      if info._dedupeIDs then
        excluded = false
        for i = 1, #info._dedupeIDs do
          if exset[info._dedupeIDs[i]] then
            excluded = true
            break
          end
        end
      ...
      if info._defaultChecked then
        if exset[info.id] == nil then
          excluded = true
        else
          excluded = exset[info.id] and true or false
        end
      end
    ...
    local function placeRowButtons(rowFrame, keys)
      ...
      for _, k in ipairs(keys) do
        ...
        tabButtons[k] = b
        if prev then
          b:SetPoint("LEFT", prev, "RIGHT", THEME.tabGap, 0)
        else
          b:SetPoint("LEFT", rowFrame, "LEFT", 0, 0)
        end
        prev = b
      end
    end
    ...
    local function layoutRowFill(rowFrame, keys)
      ...
      local totalText = 0
      for _, k in ipairs(keys) do
        totalText = totalText + (tabButtons[k]._fs:GetStringWidth() or 0)
      end
      local count, gaps = #keys, (#keys - 1) * THEME.tabGap
      local basePads = 0
      for _, k in ipairs(keys) do
        basePads = basePads + (tabButtons[k]._basePad * 2)
      end
      ...
      for i, k in ipairs(keys) do
        ...
        if i == 1 then
          ...
          b:SetPoint("LEFT", tabButtons[keys[i - 1]], "RIGHT", THEME.tabGap, 0)
        end
      end
    ...
    local function setTabsRowOrder(selectedKey)
      local inA = {}
      for _, k in ipairs(rowAKeys) do
        inA[k] = true
      end
      local selectedIsA = inA[selectedKey] == true

      if selectedIsA then
        tabRowTop:ClearAllPoints()
        tabRowBot:ClearAllPoints()
        tabRowTop:SetPoint("TOPLEFT", tabsArea, "TOPLEFT", 0, 0)
        tabRowTop:SetPoint("TOPRIGHT", tabsArea, "TOPRIGHT", 0, 0)
        tabRowBot:SetPoint("TOPLEFT", tabsArea, "TOPLEFT", 0, -(THEME.tabH + THEME.tabGap))
        tabRowBot:SetPoint("TOPRIGHT", tabsArea, "TOPRIGHT", 0, -(THEME.tabH + THEME.tabGap))
    ...
    local function collectForTab(tabKey)
      if tabKey == "FOOD" or tabKey == "FOOD_HEARTY" then
        local src = getCategorySource("FOOD")
        ...
        sortItems(hearty)
        if tabKey == "FOOD" then
          return { { label = "Food", items = plain } }
        end
        if tabKey == "FOOD_HEARTY" then
          return { { label = "Hearty Food", items = hearty } }
        end
        return {}
      elseif tabKey == "FLASK" then
        ...
        local blocks = {}
        if #normal > 0 then
          blocks[#blocks + 1] = { label = "Flasks", items = normal }
        end
        if #fleeting > 0 then
          blocks[#blocks + 1] = { label = "Fleeting Flasks", items = fleeting }
        end
        return blocks
      ...
      elseif tabKey == "WEAPON_ENCHANTS" then
        ...
        local idxBW
        for i = 1, #combined do
          if (combined[i].name or ""):lower() == "bubbling wax" then
            idxBW = i
            break
          end
        end
        if idxBW then
          local bw = table.remove(combined, idxBW)
          combined[#combined + 1] = bw
        end
      ...
      elseif tabKey == "PETS" then
        ...
        local hunterItems = {
          { id = -7001, isItem = false, name = "Hunter Pets", icon = 132161, tipSpellID = 883 },
          {
            ...
            isItem = false,
            spellID = 982,
            name = (C_Spell.GetSpellInfo and (C_Spell.GetSpellInfo(982) or {}).name) or "Revive Pet",
            icon = select(1, GetSpellIconSafe(982)),
          },
        }
        ...
        do
          ...
          local dkItems =
            { { id = sid, isItem = false, spellID = sid, name = nm, icon = select(1, GetSpellIconSafe(sid)) } }
          sortItems(dkItems)
      ...
      elseif tabKey == "UTILITY" then
        local items = {
          ...
          --{ id = 1231411, isItem = false, name = "Recuperate", icon = 136074, spellID = 1231411 },
        }
        sortItems(items)
    ...
    local function rebuildList(activeKeyForList)
      ...
      for i = 1, #blocks do
        local block = blocks[i]
        local _, y2 = makeHeader(y, block.label, block.items, activeKeyForList)
        y = y2
    ...
    scroll:HookScript("OnSizeChanged", function()
      C_Timer.After(0, function()
        ...
        local nameW = calcNameWidth()
        for i = 1, #lines do
          if lines[i] and lines[i]._name then
            lines[i]._name:SetWidth(nameW)
          end
        end
        if bar and bar.UpdateThumb then
          bar:UpdateThumb(scroll:GetHeight(), contentFrame:GetHeight())
