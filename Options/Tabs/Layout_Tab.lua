...
local CATEGORY_LABELS = {
  ...
  MAIN_HAND = "Main Hand",
  OFF_HAND = "Off Hand",
  CASTABLE_WEAPON_ENCHANTS = "Castable Weapon Enchants",
  DK_WEAPON_ENCHANTS = "DK Enchants",
  ROGUE_POISONS = "Poisons",
  AUGMENT_RUNE = "Runes",
  RAID_BUFFS = "Raid Buffs",
  SHAMAN_SHIELDS = "Shaman Shields",
  PETS = "Pets",
...
local function BuildOrderSection(content, Row)
  ...
  local hasRT = false
  for i = 1, #defaults do
    if defaults[i] == "TRINKETS" then
      hasRT = true
      break
    end
  end
  if not hasRT then
    defaults[#defaults + 1] = "TRINKETS"
  end
  ...
  local function BuildAppearanceMap()
    return {
...

      {
        ...
        centerBelow = true,
        noReset = true,
        yOffset = GLOW_CHECKBOX_YOFFSET,
      },

      {
        type = "numsel",
        key = "timerSize",
        label = "Center Text Size",
        min = bounds.cMin,
        max = bounds.cMax,
        step = 1,
        col = 2,
