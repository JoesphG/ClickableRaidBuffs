...
local function Confirm(msg, fn)
  StaticPopup_Show(POP_KEY, msg or "Reset value?", nil, fn)
end
...
local function makeArrow(parent, side)
  ...
  b:SetScript("OnEnter", function(self)
    if self:IsEnabled() then
      self._tex:SetAtlas(self._atlasEnabled, true)
    end
  end)
  b:SetScript("OnLeave", function(self)
    self._tex:SetAtlas(self._atlasDisabled, true)
  end)
  return b
end
...
function ns.NumberSelect.Create(parent, args)
  ...
  local function anyMouseOver()
    return holder:IsMouseOver()
      or edit:IsMouseOver()
      or left:IsMouseOver()
      or right:IsMouseOver()
      or reset:IsMouseOver()
  end
  ...
  reset:SetScript("OnClick", function()
    Confirm(("Reset %s to default?"):format(args.label or "value"), function()
      apply(defV)
    end)
  end)
  ...
  local function relayout()
    ...
    if labelH <= 0 then
      labelH = (O and O.SIZE_LABEL) or 14
    end
  ...
  function holder:SetEnabled(on)
    ...
    left:SetEnabled(self._enabled)
    right:SetEnabled(self._enabled)
  ...
  function holder:SetValue(v, silent)
    apply(v, silent)
  end
  function holder:GetValue()
    return value
  end
  function holder:SetRange(min2, max2)
    minV, maxV = min2 or minV, max2 or maxV
  end
  function holder:SetStep(step2)
    step = step2 or step
  end
  function holder:SetDefault(def)
    defV = def or defV
  end
  function holder:LabelText(t)
    lab:SetText(t or "")
  end
