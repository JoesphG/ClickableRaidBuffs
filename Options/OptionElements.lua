...
local THEME = {
  ...
  sizeLabel = function()
    return (O.SIZE_LABEL or 14)
  end,
  sizeEdit = function()
    return (O.SIZE_EDITBOX or 14)
  end,
  ...
  checkboxBox = function()
    return (O.TEXT_CHECKBOX_W or 20)
  end,
  swatchW = function()
    return (O.TEXT_SWATCH_W or 32)
  end,
  swatchH = function()
    return (O.TEXT_SWATCH_H or 20)
  end,
...
local function NotifyChanged()
  ...
  C_Timer.After(0.05, function()
    ...
    if
      ns
      and (
        _safeCall(ns.RequestRebuild)
        or _safeCall(ns.RebuildDisplayables)
...
local function _raiseAbove(btn, frames)
  ...
  if p and p.GetFrameStrata then
    btn:SetFrameStrata(p:GetFrameStrata() or "MEDIUM")
  end
end
...
local function _restoreTooltipFont()
  ...
  restore(GameTooltipText)
  restore(GameTooltipHeaderText)
  restore(GameTooltipTextSmall)
end
...
function OE.NewResetButton(parent)
  ...
  b:SetScript("OnShow", function(self)
    _raiseAbove(self, { self:GetParent() })
  end)
...
local function beginTriple(parent, opts)
  ...
  function ctx:_ensureRow(r)
    if not self.occupied[r] then
      self.occupied[r] = { false, false, false }
    end
    if not self.rowsH[r] then
      self.rowsH[r] = self.rowH
    end
  end
  ...
  function ctx:_mark(r, col, span, h)
    ...
    local total = 0
    for i = 1, #self.rowsH do
      total = total + self.rowsH[i] + (i > 1 and self.rowGap or 0)
    end
  ...
  function ctx:_reflow()
    ...
    local rowTop = {}
    local acc = 0
    for r = 1, #self.rowsH do
      rowTop[r] = -acc
      acc = acc + self.rowsH[r] + self.rowGap
    end
...
local function buildTextColorCell(parent, args)
  ...
  local function applyColor(c)
    ...
    if ns.SetTextColor and args.which then
      ...
      if w == "top" or w == "bottom" or w == "center" or w == "timer" or w == "corner" then
        ns.SetTextColor(w, c.r, c.g, c.b, c.a)
      end
    end
  ...
  sw:SetScript("OnClick", function()
    ...
    ColorPickerFrame:SetupColorPickerAndShow({
      ...
      cancelFunc = function(prev)
        if prev then
          applyColor({ r = prev.r, g = prev.g, b = prev.b, a = prev.opacity or 1 })
        end
      end,
    })
  end)
  ...
  reset:SetScript("OnClick", function()
    ConfirmReset(function()
      applyColor(def)
    end, args.label or (args.colorKey or "Color"))
  end)
...
local function buildFontPickerCell(parent, args)
  ...
  local function applyFont(name)
    ...
    ddText:SetText(name)
    if O and O.OnFontChanged then
      O.OnFontChanged(name)
    end
    NotifyChanged()
  end
  ...
  scroll:SetPoint("BOTTOMRIGHT", -(4 + 18), 4)
  ...
  local function rebuildMenu()
    ...
    if bar.UpdateThumb then
      bar:UpdateThumb(scroll:GetHeight(), content:GetHeight())
    end
    if bar.SetEnabledState then
...
function OE.SingleColumn(parent, buildFn)
  ...
  function api:FontPicker(args)
    local c = buildFontPickerCell(flow.holder, args)
    flow:_place(c, 72)
    return c
  end
  function api:TextColor(args)
    local c = buildTextColorCell(flow.holder, args)
    flow:_place(c, THEME.rowH)
    return c
  end
  function api:Checkbox(args)
    local cell = CreateFrame("Frame", nil, flow.holder)
    cell:EnableMouse(true)
    local lab = cell:CreateFontString(nil, "ARTWORK", "GameFontHighlight")
    ...
    cb:SetPoint("LEFT", lab, "RIGHT", 10, 0)
    local reset = OE.NewResetButton(cell)
    reset:SetPoint("TOPLEFT", cb, "BOTTOMLEFT", 0, -4)
    local d = DB()
    local key = args.key
    local def = (defaults[key] ~= false)
    cb._onToggle = function(_, v)
      d[key] = v
      if args.onChange then
        args.onChange(v)
      end
    ...
    reset:SetScript("OnClick", function()
      ConfirmReset(function()
        ...
        NotifyChanged()
      end, args.label or key)
    end)
    InstallHoverReveal(reset, { cell, cb, lab })
    _raiseAbove(reset, { cell, cb })
...
function OE.DualColumn(parent, buildFn)
  ...
  local api = {}
  function api:TextColor(args)
    local c = buildTextColorCell(grid.holder, args)
    grid:_place(c)
    return c
  end
  if type(buildFn) == "function" then
...
function OE.TripleColumn(parent, buildFn)
  ...
  function api:FontPicker(args)
    local c = buildFontPickerCell(grid.holder, args)
    grid:_placeWithSpec(c, args or {})
    return c
  end
  function api:TextColor(args)
    local c = buildTextColorCell(grid.holder, args)
  ...
  function api:Checkbox(args)
    local cell = CreateFrame("Frame", nil, grid.holder)
    cell:EnableMouse(true)
    local lab = cell:CreateFontString(nil, "ARTWORK", "GameFontHighlight")
    lab:SetFont(THEME.fontPath(), THEME.sizeLabel(), "")
    local cb = NewCheckbox(cell)
    ...
    local function layoutCentered()
      cb:ClearAllPoints()
      cb:SetPoint("TOP", cell, "TOP", 0, -6 + yOff)
      lab:ClearAllPoints()
      lab:SetPoint("TOP", cb, "BOTTOM", 0, -4)
      lab:SetJustifyH("CENTER")
    end
    local function layoutInline()
      lab:ClearAllPoints()
      lab:SetPoint("LEFT", cell, "LEFT", THEME.leftPad, yOff)
