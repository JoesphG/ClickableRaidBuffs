...
local function applyFlags(wantOutline)
  return (wantOutline and "OUTLINE") or ""
end
...
local function parentOf(fs)
  return (fs and fs.GetParent) and fs:GetParent() or nil
end
local function isCenterFS(fs)
  local p = parentOf(fs)
  return p and p.centerText == fs
end
local function isTopFS(fs)
  local p = parentOf(fs)
  return p and p.topText == fs
end
local function isBottomFS(fs)
  local p = parentOf(fs)
  return p and p.bottomText == fs
end
local function isTimerFS(fs)
  local p = parentOf(fs)
  return p and p.timerText == fs
end
local function isCornerFS(fs)
  local p = parentOf(fs)
  return p and p.cornerText == fs
end
...
local function pickCenterOutline()
  ...
  if db.centerOutline ~= nil then
    return db.centerOutline ~= false
  end
  if db.timerOutline ~= nil then
    return db.timerOutline ~= false
  end
  if db.bottomOutline ~= nil then
    return db.bottomOutline ~= false
  end
  return true
end
...
local function pickTopSize(btn)
  return scaled(DB().topSize or 14, btn)
end
local function pickTopOutline()
  local v = DB().topOutline
  return (v ~= false)
end
local function pickTopColor()
  return colorOr(DB().topTextColor, 1, 1, 1, 1)
end

local function pickBottomSize(btn)
  return scaled(DB().bottomSize or 14, btn)
end
local function pickBottomOutline()
  local v = DB().bottomOutline
  return (v ~= false)
end
local function pickBottomColor()
  return colorOr(DB().bottomTextColor, 1, 1, 1, 1)
end

local function pickTimerSize(btn)
  return pickBottomSize(btn)
end
local function pickTimerOutline()
  return pickBottomOutline()
end
local function pickTimerColor()
  return pickBottomColor()
end

local function pickCornerSize(btn)
  return scaled(DB().cornerSize or DB().bottomSize or 12, btn)
end
local function pickCornerOutline()
  local v = DB().cornerOutline
  return (v ~= false)
end
local function pickCornerColor()
  return colorOr(DB().cornerTextColor, 1, 1, 1, 1)
end
...
local function styleForFS(fs)
  ...
  if isCenterFS(fs) then
    styleCenter(fs)
    return
  end
  if isTimerFS(fs) then
    styleTimer(fs)
    return
  end
  if isTopFS(fs) then
  ...
  if isBottomFS(fs) then
    styleBottom(fs)
    return
  end
  if isCornerFS(fs) then
    styleCorner(fs)
    return
  end
end
...
local function EnsureUFSHook()
  ...
  if ns._textstyles_wrapped_ufs_ptr == cur then
    return
  end
...
local function RestyleButtonTexts(btn)
  ...
  if btn.centerText then
    styleForFS(btn.centerText)
  end
  if btn.timerText then
    styleForFS(btn.timerText)
  end
  if btn.topText then
    styleForFS(btn.topText)
  end
  if btn.bottomText then
    styleForFS(btn.bottomText)
  end
  if btn.cornerText then
    styleForFS(btn.cornerText)
  end
end
...
local function EnsureRenderHook()
  if ns._textstyles_renderWrapped then
    return
  end
...
local function StyleFingerprint()
  ...
  local f = {
    ...
    S(db.cornerSize),
    S(pickCenterOutline()),
    S(DB().topOutline ~= false),
    S(DB().bottomOutline ~= false),
    S(DB().cornerOutline ~= false),
    CenterColorS(),
