...
local function Build()
  ...
  for key, row in pairs(tbl) do
    repeat
      if type(row) ~= "table" or not row.spellID or not row.buffID or not row.buffID[1] then
        break
      end
      ...
      if row.cat == "LETHAL" then
        lethalOrder = lethalOrder + 1
      else
        nonOrder = nonOrder + 1
      end
    until true
...
local function EnsureThresholdWatcher()
  if ns._roguePoisonsThresholdWatcher then
    return
  end
