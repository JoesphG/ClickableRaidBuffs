...
function ns.IsExcluded(id)
  ...
  if ex and ex[id] then
    return true
  end
  if rb and rb[id] then
    return true
  end
  return false
end
...
function ns.IsDisplayableExcluded(cat, entry)
  ...
  local useRaidBuffSet = (
    cat == "RAID_BUFFS"
    or cat == "CLASS_ABILITIES"
    or cat == "TRINKET_RB"
    or cat == "SHAMAN_SHIELDS"
  )
...
local function _maybeIDFromFrame(f)
  ...
  if f._data then
    if f._data.itemID then
      return f._data.itemID
    end
    if f._data.spellID then
      return f._data.spellID
    end
    if f._data.id then
      return f._data.id
    end
  end
  if f.key and type(f.key) ~= "string" then
    return f.key
  end
  return nil
end
...
local function _wrapOnce(tbl, key, flagKey, post)
  if not tbl or not key or tbl[flagKey] then
    return
  end
  local orig = tbl[key]
...
_loginF:SetScript("OnEvent", function()
  C_Timer.After(0, _pruneRenderCollections)
end)
