...
local miniButton = LDB:NewDataObject("ClickableRaidBuffs", {
  ...
  OnTooltipShow = function(tooltip)
    ...
    tooltip:AddLine("|cff00ccffClickable Raid Buffs|r |cffff7d0Ffunki.gg|r")
    tooltip:AddLine(" ")
...
function ClickableRaidBuffs:RefreshMinimapIcon()
  if not (LDI and self.minimapDB and self.minimapDB.profile) then
    return
  end
...
function ClickableRaidBuffs:OnInitialize()
  ...
  if legacy then
    ...
    if dst.hide == nil and legacy.hide ~= nil then
      dst.hide = legacy.hide
    end
    if dst.minimapPos == nil and legacy.minimapPos ~= nil then
      dst.minimapPos = legacy.minimapPos
    end
    if dst.lock == nil and legacy.lock ~= nil then
      dst.lock = legacy.lock
    end
  end
...
function ns.Minimap_Show()
  if ClickableRaidBuffs then
    ClickableRaidBuffs:ToggleMinimapButton(false)
  end
end
function ns.Minimap_Hide()
  if ClickableRaidBuffs then
    ClickableRaidBuffs:ToggleMinimapButton(true)
  end
end
function ns.Minimap_Toggle()
  if not (ClickableRaidBuffs and ClickableRaidBuffs.minimapDB and ClickableRaidBuffs.minimapDB.profile) then
    return
  end
  local hide = (ClickableRaidBuffs.minimapDB.profile.minimap and ClickableRaidBuffs.minimapDB.profile.minimap.hide)
      and true
    or false
