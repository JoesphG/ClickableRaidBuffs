...
local function refreshCooldowns()
  if type(ns.Cooldown_RefreshAll) == "function" then
    ns.Cooldown_RefreshAll()
  end
end
...
local function postCatchup()
  ...
  if type(ns.RecomputeEatingState) == "function" then
    ns.RecomputeEatingState()
  end
  if type(ns.UpdateFlaskState) == "function" then
    ns.UpdateFlaskState()
  end
  if type(ns.Cosmetics_OnPlayerAura) == "function" then
    ns.Cosmetics_OnPlayerAura("player")
  end
  if type(ns.UpdateAugmentRunes) == "function" then
    ns.UpdateAugmentRunes()
  end
  if type(ns.RaidBuffTargeting_OnRosterChanged) == "function" then
    ns.RaidBuffTargeting_OnRosterChanged()
  end
  ...
  if type(ns.Durability_RefreshChosenMount) == "function" then
    ns.Durability_RefreshChosenMount()
  end
  if type(ns.Durability_Rebuild) == "function" then
  ...
  if type(ns.Delves_Recompute) == "function" then
    ns.Delves_Recompute()
  end
...
local function process_UNIT_AURA(unit, updateInfo)
  if unit == "player" then
    if type(ns.FoodStatus_OnUnitAura) == "function" then
      ns.FoodStatus_OnUnitAura(unit, updateInfo)
    end
    if type(ns.UpdateFlaskState) == "function" then
      ns.UpdateFlaskState()
    end
    if type(ns.Cosmetics_OnPlayerAura) == "function" then
      ns.Cosmetics_OnPlayerAura(unit, updateInfo)
    end
    ...
    if type(ns.FixedTarget_OnUnitAura) == "function" and ns.FixedTarget_OnUnitAura(unit, updateInfo) then
      armPoke(0)
    end
    if type(ns.RoguePoisons_OnUnitAura) == "function" and ns.RoguePoisons_OnUnitAura(unit, updateInfo) then
      armPoke(0)
    end
    if type(ns.ShamanShields_OnUnitAura) == "function" and ns.ShamanShields_OnUnitAura(unit, updateInfo) then
      armPoke(0)
    end
  ...
  elseif not auraBuf.timer then
    ...
    auraBuf.timer = C_Timer.NewTimer(delay, function()
      if not locked() then
        ...
        auraBuf.timer = C_Timer.NewTimer(0.05, flushAuras)
      end
    end)
  end
end
...
local function flushCLEU()
  ...
  if cleuBuf.pending then
    ...
    if type(ns.Healthstone_OnCombatLogEventUnfiltered) == "function" then
      if ns.Healthstone_OnCombatLogEventUnfiltered() then
        any = true
      end
    end
    if type(ns.Trinkets_OnCombatLogEventUnfiltered) == "function" then
      if ns.Trinkets_OnCombatLogEventUnfiltered() then
        any = true
      end
    end
    if any then
  ...
  elseif not cleuBuf.timer then
    cleuBuf.timer = C_Timer.NewTimer(THROTTLE - dt, function()
      if not locked() then
        ...
        cleuBuf.timer = C_Timer.NewTimer(0.05, flushCLEU)
      end
    end)
  end
end
  ...
  elseif not uscsBuf.timer then
    uscsBuf.timer = C_Timer.NewTimer(THROTTLE - dt, function()
      if not locked() then
        ...
        uscsBuf.timer = C_Timer.NewTimer(0.05, flushUSCS)
      end
    end)
  end
end
...
function ns.BypassEventThrottle()
  if auraBuf.timer then
    auraBuf.timer:Cancel()
    auraBuf.timer = nil
  end
  if cleuBuf.timer then
    cleuBuf.timer:Cancel()
    cleuBuf.timer = nil
  end
  if uscsBuf.timer then
    uscsBuf.timer:Cancel()
    uscsBuf.timer = nil
  end
  if not locked() then
    if auraBuf.pendingPlayer or next(auraBuf.units) then
      flushAuras()
    end
    if cleuBuf.pending then
      flushCLEU()
...
f:SetScript("OnEvent", function(_, event, ...)
  if event == "PLAYER_REGEN_DISABLED" then
    ns._inCombat = true
    if type(ns.Healthstone_OnRegenDisabled) == "function" then
      if ns.Healthstone_OnRegenDisabled() then
        armPoke(debounce)
      end
    end
    if type(ns.Pets_OnRegenDisabled) == "function" then
      if ns.Pets_OnRegenDisabled() then
        armPoke(debounce)
      end
    end
    if type(ns.RoguePoisons_OnRegenDisabled) == "function" then
      if ns.RoguePoisons_OnRegenDisabled() then
        armPoke(debounce)
      end
    end
    return
  elseif event == "PLAYER_REGEN_ENABLED" then
    ns._inCombat = false
    if type(ns.Healthstone_OnRegenEnabled) == "function" then
      if ns.Healthstone_OnRegenEnabled() then
        armPoke(debounce)
      end
    end
    if type(ns.Pets_OnRegenEnabled) == "function" then
      if ns.Pets_OnRegenEnabled() then
        armPoke(debounce)
      end
    end
    if type(ns.RoguePoisons_OnRegenEnabled) == "function" then
      if ns.RoguePoisons_OnRegenEnabled() then
        armPoke(debounce)
      end
    end
    if type(ns.ShamanShields_OnRegenEnabled) == "function" then
      if ns.ShamanShields_OnRegenEnabled() then
        armPoke(debounce)
      end
    end
    maybeUpdateWeaponEnchants()
    if not inEncounter() and not isDead() then
      postCatchup()
    end
    return
  ...
  elseif event == "ENCOUNTER_END" then
    ...
    if type(ns.Healthstone_OnEncounterEnd) == "function" then
      if ns.Healthstone_OnEncounterEnd() then
        armPoke(debounce)
      end
    end
    if not isDead() then
  ...
  elseif event == "PLAYER_DEAD" then
    ns._isDead = true
    if type(ns.Healthstone_OnPlayerDead) == "function" then
      if ns.Healthstone_OnPlayerDead() then
        armPoke(debounce)
      end
    end
    if type(ns.RoguePoisons_OnPlayerDead) == "function" then
      if ns.RoguePoisons_OnPlayerDead() then
        armPoke(debounce)
      end
    end
  ...
  if event == "PLAYER_ENTERING_WORLD" then
    ...
    if type(ns.RaidBuffTargeting_OnPEW) == "function" then
      ns.RaidBuffTargeting_OnPEW()
    end
    if type(ns.MythicPlus_Recompute) == "function" then
      ns.MythicPlus_Recompute()
    end
    ...
    if type(ns.Healthstone_OnPEW) == "function" then
      if ns.Healthstone_OnPEW() then
        armPoke(debounce)
      end
    end
    if type(ns.Pets_OnPEW) == "function" then
      if ns.Pets_OnPEW() then
        armPoke(debounce)
      end
    end
    if type(ns.RoguePoisons_OnPEW) == "function" then
      if ns.RoguePoisons_OnPEW() then
        armPoke(debounce)
      end
    end
    if type(ns.ShamanShields_OnPEW) == "function" then
      if ns.ShamanShields_OnPEW() then
        armPoke(debounce)
      end
    end
    if type(ns.InitRangeGate) == "function" then
      ns.InitRangeGate()
    end
    if type(ns.RangeGate_OnRosterOrSpellsChanged) == "function" then
      ns.RangeGate_OnRosterOrSpellsChanged()
    end
    if type(ns.Trinkets_Rebuild) == "function" then
      ns.Trinkets_Rebuild()
    end
    if type(ns.Trinkets_RebuildWatch) == "function" then
    ...
    if not ns._mergedScan then
      ...
      if type(orig) == "function" then
        _G.scanRaidBuffs = function(...)
          orig(...)
          if type(ns.Trinkets_Scan) == "function" then
            ns.Trinkets_Scan()
          end
        end
        ns._mergedScan = true
  ...
  if
    ...
    or event == "PLAYER_ENTERING_BATTLEGROUND"
  then
    if type(ns.Recuperate_Recompute) == "function" then
      ns.Recuperate_Recompute()
    end
    ...
    if event == "PLAYER_UPDATE_RESTING" then
      if type(ns.Healthstone_OnPlayerUpdateResting) == "function" then
        if ns.Healthstone_OnPlayerUpdateResting() then
          armPoke(debounce)
        end
      end
      if type(ns.ShamanShields_OnPlayerUpdateResting) == "function" then
        if ns.ShamanShields_OnPlayerUpdateResting() then
          armPoke(debounce)
        end
      end
    end
    if
      (
        ...
        or event == "PLAYER_DIFFICULTY_CHANGED"
        or event == "PLAYER_ENTERING_BATTLEGROUND"
      ) and type(ns.Healthstone_OnZoneChanged) == "function"
    then
      if ns.Healthstone_OnZoneChanged() then
        armPoke(debounce)
      end
    end
    if type(ns.Trinkets_Rebuild) == "function" then
  ...
  if event == "UNIT_INVENTORY_CHANGED" then
    local unit = ...
    if unit == "player" then
      if type(ns.MarkEnchantsDirty) == "function" then
        ns.MarkEnchantsDirty()
      end
      if type(ns.MarkBagsDirty) == "function" then
        ns.MarkBagsDirty()
      end
  ...
  if event == "WEAPON_ENCHANT_CHANGED" or event == "PLAYER_SPECIALIZATION_CHANGED" or event == "SPELLS_CHANGED" then
    ...
    if event == "PLAYER_SPECIALIZATION_CHANGED" then
      if type(ns.RoguePoisons_OnSpecializationChanged) == "function" then
        if ns.RoguePoisons_OnSpecializationChanged() then
          armPoke(debounce)
        end
      end
    end
  ...
  if event == "PLAYER_MOUNT_DISPLAY_CHANGED" then
    if type(ns.Pets_OnMountDisplayChanged) == "function" then
      if ns.Pets_OnMountDisplayChanged() then
        armPoke(debounce)
      end
    end
    return
  ...
  if event == "PET_STABLE_UPDATE" then
    if type(ns.Pets_OnPetStableUpdate) == "function" then
      if ns.Pets_OnPetStableUpdate() then
        armPoke(debounce)
      end
    end
    return
  ...
  if event == "BAG_UPDATE" then
    ...
    if type(ns.Healthstone_OnBagUpdate) == "function" then
      if ns.Healthstone_OnBagUpdate() then
        armPoke(debounce)
      end
    end
    return
  elseif event == "BAG_UPDATE_DELAYED" then
    ...
    if type(ns.Healthstone_OnBagUpdateDelayed) == "function" then
      if ns.Healthstone_OnBagUpdateDelayed() then
        armPoke(debounce)
      end
    end
    armPoke(debounce)
  ...
  if event == "BAG_UPDATE_COOLDOWN" or event == "SPELL_UPDATE_COOLDOWN" then
    if event == "SPELL_UPDATE_COOLDOWN" and type(ns.Pets_OnSpellUpdateCooldown) == "function" then
      if ns.Pets_OnSpellUpdateCooldown() then
        armPoke(debounce)
      end
    end
  ...
  if event == "CHALLENGE_MODE_MAPS_UPDATE" then
    if type(ns.MythicPlus_OnMapsUpdate) == "function" then
      ns.MythicPlus_OnMapsUpdate()
    end
    return
  end
