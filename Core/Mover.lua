...
local function UpdateMoverSize()
  if not ns._mover or not parent then
    return
  end
  ...
  if show then
    if not ns._mover then
        ...
        if button == "LeftButton" then
          ...
          self:SetScript("OnUpdate", function()
            if self.isDragging then
              ...
              local pos = GetPos()
              pos.x, pos.y =
                math.floor(newX - UIParent:GetWidth() / 2 + 0.5), math.floor(newY - UIParent:GetHeight() / 2 + 0.5)
              ApplyPosition()
            end
          end)
        end
      ...
      local function layoutTextWidth(b)
        if not b or not b.GetFontString then
          return
        end
