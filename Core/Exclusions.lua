...
local function rebuild()
  ...
  for id, v in pairs(a) do
    if v then
      ai[tonumber(id) or id] = true
    end
  end
  for id, v in pairs(r) do
    if v then
      ri[tonumber(id) or id] = true
    end
  end
...
local function ensure()
  ...
  local a, r = getSets()
  if a ~= state.a_ptr or r ~= state.r_ptr then
    rebuild()
  end
end
...
function M.IsExcluded(cat, entry)
  ...
  if not key then
    local disp = _G.clickableRaidBuffCache
      and _G.clickableRaidBuffCache.displayable
      and _G.clickableRaidBuffCache.displayable[cat]
