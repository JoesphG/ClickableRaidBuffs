...
for i = 1, #DEFAULT_ORDER do
  allowed[DEFAULT_ORDER[i]] = true
end

function ns.GetCategoryOrderDefaults()
  local out = {}
  for i = 1, #DEFAULT_ORDER do
    out[i] = DEFAULT_ORDER[i]
  end
  return out
end
...
local function EffectiveOrder()
  ...
  for i = 1, #saved do
    ...
    if allowed[c] and not seen[c] then
      seen[c] = true
      out[#out + 1] = c
    end
  end
...
function ns.SetCategoryOrder(order)
  ...
  if type(order) ~= "table" then
    if ns.DebugPrint then
      ns.DebugPrint("SetCategoryOrder ignored: expected table, got " .. type(order))
    end
    return
  end
...
local function EntryStableKey(entry)
  if entry and entry.category == "PETS" then
    ...
    if entry.itemID then
      return "p:" .. hint .. ":i:" .. entry.itemID
    end
    if entry.spellID then
      return "p:" .. hint .. ":s:" .. entry.spellID
    end
