...
local RAID_PALADIN = {
  ...
  [1465] = {
    ...
    gates = { "instance" },
    infinite = true,
  }, --Instance only version that encourages swapping to Devo if no Devo applied
}
...
do
  local function merge(src)
    for id, e in pairs(src) do
      ClickableRaidData.ALL_RAID_BUFFS[id] = e
    end
  end
  merge(RAID_WARRIOR)
  merge(RAID_PALADIN)
...
if ns and ns.RebuildRaidBuffWatch then
